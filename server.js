...
app.get("/about", (req, res) => {
  res.render("about", { ...baseData, pageTitle: "About" });
});

app.get("/services", (req, res) => {
  res.render("services", { ...baseData, pageTitle: "Services", accreditedParameters, featuredServices, serviceCategories });
});

app.get("/accreditation", (req, res) => {
  res.render("accreditation", { ...baseData, pageTitle: "Accreditation", accreditedParameters, galleryImages: equipmentImages.slice(0, 8), capabilityHighlights, certificateExtracts, certificateScopeExtracts });
});

app.get("/contact", (req, res) => {
  res.render("contact", { ...baseData, pageTitle: "Contact", sent: false });
});

app.post("/contact", (req, res) => {
  res.render("contact", { ...baseData, pageTitle: "Contact", sent: true, formData: req.body });
});

app.listen(PORT, HOST, () => {
  console.log(`Qualitex website running at http://${HOST}:${PORT}`);
});
